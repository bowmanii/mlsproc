...
upper_limit1 <- 371.000
lower_limit1 <- 368.000
outliers_removed1 <- subset(wl_sub1, head_masl_cf <= upper_limit1 & head_masl_cf >= lower_limit1)

boxplot(wl_sub2$head_masl_cf)
z_score2 <- scale(wl_sub1$head_masl_cf) # standardizing the data
outliers2 <- wl_sub2$head_masl_cf[abs(z_score) > 3]
upper_limit2 <- 370.000
lower_limit2 <- 368.700
outliers_removed2 <- subset(wl_sub2, head_masl_cf <= upper_limit2 & head_masl_cf >= lower_limit2)

boxplot(wl_sub3$head_masl_cf)
z_score3 <- scale(wl_sub3$head_masl_cf) # standardizing the data
outliers3 <- wl_sub3$head_masl_cf[abs(z_score) > 3]
upper_limit3 <- 371.000
...
outliers5 <- wl_sub5$head_masl_cf[abs(z_score) > 3]
upper_limit5 <- 372.000
lower_limit5 <- 369.000
outliers_removed5 <- subset(wl_sub5, head_masl_cf <= upper_limit5 & head_masl_cf >= lower_limit5)

boxplot(wl_sub7$head_masl_cf)
z_score7 <- scale(wl_sub7$head_masl_cf) # standardizing the data
outliers7 <- wl_sub7$head_masl_cf[abs(z_score) > 3]
upper_limit7 <- 372.000
lower_limit7 <- 371.000
outliers_removed7 <- subset(wl_sub7, head_masl_cf <= upper_limit7 & head_masl_cf >= lower_limit7)

boxplot(wl_sub8$head_masl_cf)
z_score8 <- scale(wl_sub8$head_masl_cf) # standardizing the data
outliers8 <- wl_sub8$head_masl_cf[abs(z_score) > 3]
upper_limit8 <- 372.500
lower_limit8 <- 370.000
outliers_removed8 <- subset(wl_sub8, head_masl_cf <= upper_limit8 & head_masl_cf >= lower_limit8)

boxplot(wl_sub10$head_masl_cf)
z_score10 <- scale(wl_sub10$head_masl_cf) # standardizing the data
outliers10 <- wl_sub10$head_masl_cf[abs(z_score) > 3]
upper_limit10 <- 375.000
...
s1 <- subplot(p_wl, p_baro, p_rain, scw, shareX = TRUE, nrows = 4, heights = c(0.52, 0.08, 0.1, 0.3))%>%
  layout(
    title = list(text = "ELR1-QA/QB: MLS Ports", # ELR1-QA, ELR1-QB, ELR1-QA/QB, ELR2-QA
                 y = 0.98,
                 font = list(size = 18)), 
